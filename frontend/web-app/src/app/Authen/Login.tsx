...
function Login() {
  ...
  useEffect(() => {
    ...
    if (storedUserId) {
      navigate(RoutePath.userDetail(storedUserId));
    }
    if (storedUserAdmin) {
  ...
  const handleLogin = async () => {
    if (phone == admin) {
      ...
      try {
        const response = await agent.user.login(phone);
    ...
    >
      <div className="formLogin">
        <div style={{ display: "flex", justifyContent: "center" }}>
          <p style={{ fontSize: 40, fontWeight: 600 }}>เข้าสู่ระบบ</p>
        </div>
        ...
        <button className="button-submitLogin" onClick={handleLogin}>
          เข้าสู่ระบบ
        </button>
        <div style={{ textAlign: "center", marginTop: "10px" }}>
          <span>
            ...
            <span
              onClick={handleShow}
              style={{ color: "#3946fd", fontWeight: 600, cursor: "pointer" }}
            >
              ลงทะเบียนเลย!
            </span>
          </span>
